...
# 用法:
#   ./build.sh          -> 构建默认纯净基础镜像 (deepseek-harness-docker:latest 及内置 dsh 版本标签)
#   ./build.sh --market -> 构建带预装插件清单的镜像 (deepseek-harness-docker:market 及内置 dsh 版本标签)
...
if [ "$1" = "--market" ] || [ "$1" = "-m" ] || [ "$PREINSTALL_PLUGINS" = "1" ]; then
  ...
  echo " 关联内置 DSH 版本标签: ${IMAGE_NAME}:${DSH_VERSION}-market"
  ...
  docker build \
    ...
    -t "${IMAGE_NAME}:v${DSH_VERSION}-market" \
    .
  echo ">>> 成功产出镜像: ${IMAGE_NAME}:market 与 ${IMAGE_NAME}:${DSH_VERSION}-market"
  echo ">>> 可使用 'docker compose -f docker-compose.market.yml up -d' 启动测试"
else
  echo "========================================================="
  echo " 构建默认基础镜像: ${IMAGE_NAME}:latest (默认不预装插件)"
  echo " 关联内置 DSH 版本标签: ${IMAGE_NAME}:${DSH_VERSION}"
  ...
  docker build \
    ...
    -t "${IMAGE_NAME}:v${DSH_VERSION}" \
    .
  echo ">>> 成功产出镜像: ${IMAGE_NAME}:latest 与 ${IMAGE_NAME}:${DSH_VERSION}"
